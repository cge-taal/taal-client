<script lang="ts">
  ...
  function getSettings() {
    api.getSettings(
      ...
      (error) => failure(t('notifications.failure', { error }), { duration: 5000 })
    )
  }
...
<PageWithMenu>
  <div class="island">
    ...
    <Spacer h={8} />
    <Text testId={'debug-hint-text'} size={5} color="#8F8D94" value={t(`${pageKey}.debug-hint`)} />
    <Spacer h={16} />
    <Checkbox
    ...
    <Spacer h={24} />
    <Text testId={'database-mode-label'} size={4} value={t(`${pageKey}.database-mode-label`)} />
    <Spacer h={16} />
    <div class="radio-row">
      <Radio
