<script lang="ts">
  ...
  let modeItems: { label: any; value: string }[] = []
  ...
  // api keys
  let keys: any[] = []
  let apiKeyItems: { label: any; value: any }[] = []
  let apiKey
  ...
  $: submitDisabledCommon = !apiKey || dataTransmissionInProgress || hasFileOverLimit || inProgress
  ...
  function writeDataItem(data, mimeType, filename) {
    return api.writeTransaction(
      ...
      (data) => success(t(`${pageKey}.notifications.transaction-submit-success`)),
  ...
  onMount(async () => {
    ...
    const storedKeyExists = keys.filter((key) => key.api_key === apiKey).length > 0
...
<PageWithMenu>
  <div class="island">
    ...
    <div class="sub-row">
      ...
      <Switch name="devMode" label={t(`${pageKey}.developer-mode-label`)} bind:checked={devMode} />
    </div>
    <Spacer h={24} />
    <Dropdown
      name="apiKey"
    ...
    <div class="mode-row">
      ...
      />
      {#if mode === 'encrypt'}
        <TextInput name="secret" label={t(`${pageKey}.secret-label`)} bind:value={secret} />
      {/if}
    </div>
    {#if devMode}
      <Spacer h={24} />
    ...
    <Spacer h={64} />
    <div class="buttons">
      <Button variant="ghost" size="large" disabled={submitButtonIsDisabled} on:click={reset}>
