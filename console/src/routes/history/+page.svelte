<script lang="ts">
  ...
  const getRowIconActions = (tableName, item, idField = 'id') => {
    const icons: { icon: string; type?: string; render?: string; disabled?: boolean }[] = []
    if (item?.isHash) {
      icons.push({
        icon: 'hashtag',
        render: 'icon',
  ...
  function download(tx) {
    ...
    fetch(`${PUBLIC_BASE_URL}/api/v1/transactions/${tx.id}`, {
      method: 'GET',
      headers: {
      ...
      .then((response) => {
        if (!response.ok) {
          throw new Error(t(`${pageKey}.errors.network-response`).toString())
        }
...
<PageWithMenu>
  <div class="island">
    ...
    <Spacer h={24} />
    <div class="sub-row">
      <ButtonSelect name="range" value={rangeValue} items={rangeItems} on:change={onRange} />
    </div>
    <Spacer h={24} />
    <Table
      name="history"
